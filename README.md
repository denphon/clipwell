...
- **Preview window**: open an entry's full content in a separate resizable window (centered, text selection, rich/plain toggle, clickable links); close it with Escape. A find box highlights matches in every mode (Enter / Shift+Enter to step through), and the structured tree can filter to matching entries. Images are shown at their real physical size (DPI-aware) and support scroll-to-zoom, drag-to-pan and middle-click reset.
- **Structured preview**: JSON / YAML / XML / HTML is shown as a collapsible, syntax-highlighted tree (expand/collapse all, hide empty fields). Captured-log line labels like `[OUTGOING]` / `[INCOMING]` are preserved so the direction stays visible.
...
- **Paste / Copy as Markdown or HTML**: from an entry's right-click menu, convert rich content to Markdown source or a Markdown entry to HTML, and either paste it into the active window or copy it to the clipboard.
